...
export default function AquaFlowPlanner() {
  ...
  }, [tasks, selectedDateKey, activeFlowDateStr, now, prefData])

  const overdueTasks = useMemo(() => {
    return tasks
      .filter(t => isTaskOverdue(t))
      .sort((a, b) => a.date.localeCompare(b.date))
  }, [tasks, activeFlowDateStr, now, prefData])
  ...
  const handleToggleFlowDate = (id: string) => {
    ...
    const targetDate = isCurrent ? format(subDays(selectedDate, 1), "yyyy-MM-dd") : selectedDateKey
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {overdueTasks.length > 0 && (
              <Collapsible
                ...
                className="w-full space-y-2 bg-destructive/5 rounded-2xl border border-destructive/10 overflow-hidden transition-all duration-200 hover:bg-destructive/10"
              >
                <CollapsibleTrigger asChild>
                  <Button 
                    variant="ghost" 
                    className="w-full flex items-center justify-between p-4 h-auto hover:bg-transparent text-destructive font-bold transition-colors"
                  >
